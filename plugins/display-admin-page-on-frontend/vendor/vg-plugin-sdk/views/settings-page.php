...
<div class="vg-plugin-sdk-page" style="margin-top: 20px;" id="<?php echo esc_attr('vgfpsdk_settings_' . $this->args['opt_name']); ?>">
	<div class="">
		...
		<?php if (!empty($this->settings['website'])) { ?>
			...
			<?php } ?>
			<img src="<?php echo esc_url($this->settings['logo']); ?>" class="vg-logo" <?php if( ! empty( $this->settings['logo_width'] ) ) echo 'width="' . (int) $this->settings['logo_width'] . '"'; ?>>
			<?php if (!empty($this->settings['website'])) { ?>
			</a>
		<?php } ?>
	</div>
