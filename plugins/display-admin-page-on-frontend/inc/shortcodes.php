...
if ( !class_exists( 'WPFA_Shortcodes' ) ) {
    class WPFA_Shortcodes {
        ...
        function get_logout_url( $atts = array(), $content = '' ) {
            $redirect_to = VG_Admin_To_Frontend_Obj()->get_logout_redirect_to( get_current_user_id() );
        ...
        function get_admin_page_for_frontend( $atts = array(), $content = '' ) {
            ...
            // Fix URLs if they enter /wp-admin/ or wp-admin/ as prefix by mistake
            if ( strpos( $page_url, '/wp-admin/' ) === 0 ) {
                $page_url = ltrim( $page_url, '/wp-admin/' );
            }
            if ( strpos( $page_url, 'wp-admin/' ) === 0 ) {
                $page_url = ltrim( $page_url, 'wp-admin/' );
            }
            if ( !is_user_logged_in() ) {
                ...
                ob_start();
                // Hook needed by some recaptcha plugins
            ...
            preg_match_all( '/{urlparameter:([^}]+)/', $final_url, $parameters );
            if ( !empty( $parameters ) && !empty( $parameters[1] ) ) {
                foreach ( $parameters[1] as $parameter_key ) {
                    if ( !isset( $_GET[$parameter_key] ) ) {
                        continue;
                    }
                    $final_url = str_replace( '{urlparameter:' . sanitize_text_field( $parameter_key ) . '}', sanitize_text_field( $_GET[$parameter_key] ), $final_url );
                }
            }
            ...
            if ( strpos( $final_url, 'customize.php' ) !== false ) {
                $redirect_to = VG_Admin_To_Frontend_Obj()->get_settings( 'redirect_to_frontend', home_url( '/' ) );
                $final_url = add_query_arg( 'return', $redirect_to, $final_url );
            }
            ...
            $allowed_keywords = apply_filters( 'wp_frontend_admin/allowed_keywords_in_iframe', array_merge( $allowed_keywords, array(
                'elementor',
                'microthemer',
        ...
        function get_login_form( $atts = array(), $content = '' ) {
            ...
            if ( is_user_logged_in() ) {
                ...
                if ( VG_Admin_To_Frontend_Obj()->is_frontend_dashboard_user( $user_id ) && empty( $redirect_to ) ) {
                    $redirect_to = VG_Admin_To_Frontend_Obj()->get_settings( 'redirect_to_frontend', home_url( '/' ) );
                ...
                } elseif ( $redirect_to && $redirect_to !== VG_Admin_To_Frontend_Obj()->get_current_url() ) {
                    $blog_id = WPFA_Global_Dashboard_Obj()->get_site_id_for_admin_content();
                    $redirect_to = str_replace( array(
                        '{user_site_base_url}',
                        'http://{user_site_base_url}',
                        'https://{user_site_base_url}',
                        'http://user_site_base_url',
                        'https://user_site_base_url'
                    ), get_site_url( $blog_id ), $redirect_to );
                    $current_url = VG_Admin_To_Frontend_Obj()->get_current_url();
                    if ( trailingslashit( $redirect_to ) === trailingslashit( $current_url ) ) {
                        return ob_get_clean();
                    }
                    ?>
