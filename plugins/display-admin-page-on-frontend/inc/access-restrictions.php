...
if ( !class_exists( 'WPFA_Access_Restrictions' ) ) {
    class WPFA_Access_Restrictions {
        ...
        public $last_blacklist_url_checked = null;
        ...
        function maybe_start_learning_mode() {
            ...
            if ( !empty( $_GET['wpfa_learning_mode_toggle'] ) && !empty( $_GET['wpfa_nonce'] ) && wp_verify_nonce( $_GET['wpfa_nonce'], 'wpfa' ) ) {
                ...
                VG_Admin_To_Frontend_Obj()->update_option( 'access_restrictions_learning_mode_active', ( $is_active ? false : time() ) );
            ...
            $ten_minutes_ago_timestamp = time() - MINUTE_IN_SECONDS * 10;
        ...
        function modify_whitelisted_urls_before_saving( $options ) {
            ...
            if ( function_exists( 'WC' ) && VG_Admin_To_Frontend_Obj()->is_master_user() && strpos( $all_urls, 'admin.php?page=wc-settings' ) !== false ) {
                $urls[] = '?preview_woocommerce_mail=true';
            }
            if ( strpos( $all_urls, 'upload.php' ) !== false ) {
                $urls[] = 'media-new.php';
                $urls[] = 'upload.php?mode=list';
                $urls[] = 'upload.php?mode=grid';
                $urls[] = 'upload.php?item={any_numbers}';
            }
            $urls = array_map( array(VG_Admin_To_Frontend_Obj(), 'get_admin_url_without_base'), $urls );
        ...
        function enforce_frontend_dashboard() {
            ...
            $skip_frontend_dashboard_enforcement = apply_filters( 'vg_admin_to_frontend/skip_frontend_dashboard_enforcement', $is_blacklisted || !is_user_logged_in() || !is_admin() || VG_Admin_To_Frontend_Obj()->is_master_user() || wp_doing_ajax() || wp_doing_cron(), $is_blacklisted );
        ...
        function is_page_blacklisted() {
            ...
            $this->last_blacklist_url_checked = $url_path;
            if ( $allowed ) {
                return apply_filters(
                    'vg_admin_to_frontend/is_page_blacklisted',
                    false,
        ...
        function apply_page_blacklist() {
            ...
            $default_url = add_query_arg( array(
                'vgfa_blacklisted_url' => 1,
            ), VG_Admin_To_Frontend_Obj()->get_settings( 'redirect_to_frontend', home_url( '/' ) ) );
        ...
        function allow_iframes_on_origin() {
            ...
            $is_wpbakery_editor = !is_admin() && !empty( $_GET['vc_editable'] ) && !empty( $_GET['vc_post_id'] ) && current_user_can( 'edit_post', (int) $_GET['vc_post_id'] );
            if ( is_admin() || $is_dashboard_site || $is_customizer_request || $is_beaver_builder_editor || $is_elementor_editor || $is_wpbakery_editor ) {
