...
if ( !class_exists( 'VG_Admin_To_Frontend' ) ) {
    class VG_Admin_To_Frontend {
        ...
        public static $textname = 'vg_admin_to_frontend';

        public static $dir = __DIR__;

        public static $file = __FILE__;

        public static $version = '1.22.8';

        public static $name = 'Frontend Admin';

        public static $main_admin_id_key = 'vgfa_admin_main_admin_id';

        public static $main_admins_id_key = 'vgfa_admin_main_admin_ids';

        public $allowed_urls = array();

        public $base_page_id = 'vgca_base_page_id';

        public $redirect_after_save_post = null;

        public $master_users = array();

        public $base_options = array();

        public $settings_from_file = array();
        ...
        function get_file_config_file_path() {
            return apply_filters( 'wp_frontend_admin/config_file_path', WP_PLUGIN_DIR . '/wp-frontend-admin-configuration/index.php' );
        }

        function parse_settings_from_file() {
            // If this is the dashboard site in a multitenant environment, always use the config from the local db, because the config from the file is used by tenant sites only to propagate the settings
            if ( VG_Admin_To_Frontend_Obj()->get_settings( 'environment_type', '' ) === 'dashboard_site' ) {
                return;
            }
            $config_file_path = $this->get_file_config_file_path();
            $plugin_slug = str_replace( WP_PLUGIN_DIR . '/', '', $config_file_path );
            ...
            $this->settings_from_file = $config;
        }

        function get_default_editor_for_post_type( $post_type = null ) {
            $default_editor = '';
            if ( !empty( $post_type ) ) {
                $default_editor = VG_Admin_To_Frontend_Obj()->get_settings( 'default_editor_' . $post_type, '' );
            }
        ...
        function get_system_page_ids() {
            ...
            $elementor_pages_with_wpfa_shortcode = $wpdb->get_col( "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = '_elementor_data' AND meta_value LIKE '%[vg_display_admin_page%' " );
            ...
            $system_page_ids = array_unique( array_map( 'intval', array_merge(
                $pages_with_wpfa_shortcode,
                $pages_with_wpfa_meta,
                $extra_pages,
                $elementor_pages_with_wpfa_shortcode
            ) ) );
            return apply_filters( 'wp_frontend_admin/system_page_ids', $system_page_ids );
        }

        function is_main_admin( $user_id = null ) {
            if ( !$user_id ) {
                $user_id = get_current_user_id();
            }
            $admin_ids = get_option( self::$main_admins_id_key );
            if ( empty( $admin_ids ) || !is_array( $admin_ids ) ) {
                $admin_ids = array();
            }
            $admin_ids = array_map( 'intval', $admin_ids );
            return in_array( $user_id, $admin_ids, true ) && user_can( $user_id, $this->master_capability() );
        }

        function set_main_admin_id( $user_id = null ) {
            if ( !$user_id ) {
                $user_id = get_current_user_id();
            }
            if ( !user_can( $user_id, $this->master_capability() ) ) {
                return false;
            }
            $admin_ids = get_option( self::$main_admins_id_key );
            if ( empty( $admin_ids ) || !is_array( $admin_ids ) ) {
            ...
            $admin_ids[] = $user_id;
            update_option( VG_Admin_To_Frontend::$main_admins_id_key, array_unique( $admin_ids ) );
        }

        function get_backend_urls_forced_frontend() {
            $backend_urls_forced_outside_iframe = array_filter( array_map( 'trim', explode( ',', VG_Admin_To_Frontend_Obj()->get_settings( 'backend_urls_forced_outside_iframe', '' ) ) ) );
            $backend_urls_forced_outside_iframe = apply_filters( 'wp_frontend_admin/backend_urls_forced_outside_iframe', $backend_urls_forced_outside_iframe );
            return array_values( array_filter( $backend_urls_forced_outside_iframe ) );
        }
        ...
        function get_settings( $key = null, $default = null, $skip_filter = false ) {
            ...
            $out = $options;
            if ( !empty( $key ) ) {
                if ( isset( $this->settings_from_file[$key] ) ) {
                    $out = $this->settings_from_file[$key];
                } else {
        ...
        function init() {
            $this->parse_settings_from_file();
            // Do nothing if WP is running from the command line, which has zero need for our dashboard functionality
            if ( defined( 'WP_CLI' ) && WP_CLI ) {
                return;
            }
            if ( $this->get_settings( 'excluded_network_sites_ids' ) && is_multisite() ) {
                $network_settings_page_url = network_admin_url( 'admin.php?page=vg_admin_to_frontend' );
                $referrer = $this->get_referer();
                $request_comes_from_settings_page = $referrer && $referrer === $network_settings_page_url;
                $excluded_site_ids = array_filter( array_map( 'intval', explode( ',', $this->get_settings( 'excluded_network_sites_ids' ) ) ) );
                if ( !is_network_admin() && !$request_comes_from_settings_page && !empty( $excluded_site_ids ) && in_array( get_current_blog_id(), $excluded_site_ids, true ) ) {
                    return;
                }
            }
            $this->args = array(
            ...
            if ( is_admin() ) {
                ...
                // Compatibility with Booknetic
                add_action( 'bkntc_enqueue_assets', array($this, 'cleanup_admin_page_for_frontend') );
                ...
                add_filter(
                    'vg_plugin_sdk/settings/' . VG_Admin_To_Frontend::$textname . '/get_setting',
                    array($this, 'filter_settings_page_values'),
                    10,
                    3
                );
            }
            ...
            add_filter(
                'logout_redirect',
                array($this, 'redirect_after_log_out'),
                99,
                3
            );
            load_plugin_textdomain( VG_Admin_To_Frontend::$textname, false, basename( __DIR__ ) . '/lang/' );
            add_action( 'clean_site_cache', array($this, 'delete_sites_transient') );
            // Migrate the old setting to the new settings key. This can be removed in a few months
            $old_admin_id = get_option( self::$main_admin_id_key );
            if ( $old_admin_id ) {
                update_option( self::$main_admins_id_key, array($old_admin_id) );
                delete_option( self::$main_admin_id_key );
            }
            add_filter( 'body_class', array($this, 'add_frontend_admin_body_class') );
            // Disabled the admin email check screen because it's not needed for front end dashboard users
            add_filter( 'admin_email_check_interval', '__return_false' );
        }

        public function add_frontend_admin_body_class( $classes ) {
            if ( VG_Admin_To_Frontend_Obj()->is_wpfa_page() ) {
                $classes[] = 'wp-frontend-admin-page';
            }
            return $classes;
        }

        function filter_settings_page_values( $out, $key, $default ) {
            return $this->get_settings( $key, $default );
        }

        function delete_sites_transient() {
            delete_transient( 'wpfa_network_sites' );
        }

        function get_user_option_from_master_user( $result, $option, $user ) {
            $global_user_id = (int) $this->get_settings( 'wp_screen_options_user_id' );
            if ( $global_user_id !== $user->ID ) {
                $result = get_user_option( $option, $global_user_id );
            }
            return $result;
        }
        ...
        function render_back_buttons_for_internal_pages() {
            ...
            if ( strpos( $_SERVER['REQUEST_URI'], '/post.php?post=' ) !== false ) {
                $go_to_url = esc_url( add_query_arg( 'post_type', get_post_type( (int) $_GET['post'] ), admin_url( 'edit.php' ) ) );
            }
            $go_to_url = apply_filters( 'wp_frontend_admin/back_button/url', $go_to_url );
            if ( strpos( $_SERVER['REQUEST_URI'], 'vgfa_form_submission' ) !== false && !empty( $_GET['wp_http_referer'] ) ) {
                $go_to = 'window.location.href = ' . json_encode( esc_url( remove_query_arg( array('vgfa_form_submission', 'vgfa_internal'), $_GET['wp_http_referer'] ) ) ) . ';';
            } elseif ( $go_to_url === 'history' ) {
                $go_to = 'window.history.back(2);';
            } else {
                $go_to = 'window.location.href = ' . json_encode( esc_url( $go_to_url ) ) . ';';
            }
            $allowed_button = !empty( $_POST ) || preg_match( '/(vgfa_internal|update)/', $_SERVER['REQUEST_URI'] );
            if ( !$allowed_button ) {
                return;
            }
            ?>
			<script>
				jQuery(window).on('load', function () {
					if (jQuery('html').hasClass('vgca-only-admin-content')) {
						jQuery(<?php 
            echo json_encode( sanitize_text_field( $back_button_container ) );
            ?>).prepend('<button type="button" onclick="<?php 
            echo esc_attr( $go_to );
            ?> wpFrontendAdminBackend.parentFunction(\'vgfaStartLoading\'); event.preventDefault; return false;" class="page-title-action wpfa-back-button"><span class="dashicons dashicons-arrow-left-alt2" style="height: auto"></span> <?php 
        ...
        function get_logout_redirect_to( $user_id ) {
            $redirect_to = null;
            $logout_redirection = $this->get_settings( 'logout_redirection' );
            if ( empty( $logout_redirection ) ) {
                $redirect_to = $this->get_login_url( home_url( '/' ) );
            } elseif ( $logout_redirection === 'user_home_page' ) {
                $redirect_to = WPFA_Global_Dashboard_Obj()->get_current_site_url();
            } elseif ( $logout_redirection === 'network_home_page' ) {
                $redirect_to = get_site_url( 1, '/' );
            }
            return $redirect_to;
        }

        function redirect_after_log_out( $redirect_to, $requested_redirect_to, $user ) {
            if ( $this->get_settings( 'disable_logout_redirection' ) ) {
                return $redirect_to;
            }
            if ( !empty( $_REQUEST['redirect_to'] ) ) {
                $redirect_to = esc_url_raw( $_REQUEST['redirect_to'] );
            } else {
                $redirect_to = $this->get_logout_redirect_to( $user->ID );
            }
            if ( empty( $redirect_to ) ) {
                return $redirect_to;
            }
            $redirect_to = esc_url_raw( add_query_arg( 'vgfa_redirect_after_logout', 1, $redirect_to ) );
            return $redirect_to;
        }
        ...
        function redirect_to_new_after_publish( $new_status, $old_status, $post ) {
            ...
            } elseif ( $redirect_to === 'view_post' ) {
                $url = get_permalink( $post->ID );
            }
            if ( empty( $url ) ) {
                return;
            }
            $this->redirect_after_save_post = apply_filters( 'wp_frontend_admin/redirect_to_url_after_post_published', $url, $post );
        }
        ...
        function redirect_to_new_after_post_pending_review( $new_status, $old_status, $post ) {
            ...
            $this->redirect_after_save_post = apply_filters( 'wp_frontend_admin/redirect_to_url_after_post_pending', $url, $post );
        }
        ...
        function maybe_redirect_after_post_saved( $url ) {
            ...
            if ( !wp_doing_ajax() && strpos( $url, 'vgfa_internal' ) === false && wp_get_referer() && strpos( wp_get_referer(), 'vgfa_internal' ) !== false ) {
                $url = esc_url( add_query_arg( 'vgfa_internal', 1, $url ) );
            }
        ...
        function master_capability() {
            return apply_filters( 'wp_frontend_admin/master_capability', ( is_multisite() ? 'manage_network' : 'manage_options' ) );
        }
        ...
        function is_master_user( $user_id = null ) {
            ...
            $is_master_user = apply_filters( 'wp_frontend_admin/is_master_user', null, $user_id );
            if ( is_bool( $is_master_user ) ) {
                return $is_master_user;
            }
            if ( isset( $this->master_users['user' . $user_id] ) ) {
                return $this->master_users['user' . $user_id];
            }
            $this->master_users['user' . $user_id] = ( is_multisite() ? user_can( $user_id, $this->master_capability() ) : $this->is_main_admin( $user_id ) );
            return $this->master_users['user' . $user_id];
        }
        ...
        function get_current_url( $with_port = false ) {
            // If they are using local by flywheel, the public URL does not use port but flywheel
            if ( !empty( $_SERVER['LOCALAPPDATA'] ) || !empty( $_SERVER['SERVER_ADDR'] ) && $_SERVER['SERVER_ADDR'] === '127.0.0.1' ) {
                $with_port = false;
            }
            $pageURL = 'http';
        ...
        function get_admin_url_without_base( $page_url ) {
            ...
            // Update: No longer requied, upload.php works with query strings since a recent wp update
        ...
        function get_login_url( $default = null ) {
            ...
            // We used to automatically load the login page from the current site, regardless of the site defined in the login_page_url setting
            // Now we disable it to see if it works fine for everybody
            //          if (is_multisite()) {
        ...
        function _default_wp_die_handler( $message, $title = '', $args = array() ) {
            if ( !VG_Admin_To_Frontend_Obj()->is_frontend_dashboard_user( get_current_user_id() ) ) {
        ...
        function show_message_if_page_not_allowed( $message, $title, $args ) {
            ...
            if ( !did_action( 'admin_head' ) ) {
                WPFA_Access_Restrictions_Obj()->remove_frame_options_header();
                WPFA_Access_Restrictions_Obj()->allow_iframes_on_origin();
            }
            ...
            if ( filter_var( $url, FILTER_VALIDATE_URL ) ) {
                wp_safe_redirect( esc_url( add_query_arg( 'vgfa_redirect', 'die_handler', trailingslashit( $url ) ) ) );
                exit;
            }
            if ( empty( $_GET['vgfa_source'] ) || $this->get_settings( 'disable_permissions_help_message', false ) ) {
                return $this->_default_wp_die_handler( $message, $title, $args );
            }
            $permissions_message = VG_Admin_To_Frontend_Obj()->get_settings( 'wrong_permissions_message', sprintf( __( 'Tip from WP Frontend Admin:' . '<br>1. This page requires a different user role. Please view this page as administrator and follow the instructions in the "quick settings" section to fix this. <a href="%s" target="_blank">See an screenshot</a>.<br>2. You can get help in the <a href="%s" target="_blank">live chat on our website</a> or open a support ticket via email. We will help you with the setup for free.', VG_Admin_To_Frontend::$textname ), esc_url( plugins_url( '/assets/imgs/page-not-allowed-instructions.png', VG_Admin_To_Frontend::$file ) ), 'https://wpfrontendadmin.com/contact/?utm_source=wp-admin&utm_campaign=wrong-permissions-help&utm_medium=' . (( empty( VG_Admin_To_Frontend_Obj()->allowed_urls ) ? 'free' : 'pro' )) . '-plugin' ) );
            ob_start();
        ...
        function save_quick_settings() {
            ...
            if ( !$this->is_master_user() || empty( $_REQUEST['ID'] ) || empty( $_REQUEST['_wpnonce'] ) || !wp_verify_nonce( $_REQUEST['_wpnonce'], 'vg_frontend_admin_save_quick_settings' ) ) {
                wp_send_json_error( 'You do not have permission to edit this post.', VG_Admin_To_Frontend::$textname );
            }
        ...
        function add_post_edit_link( $link ) {
            ...
            if ( $add_post_edit_link ) {
                ...
                $link = apply_filters( 'wp_frontend_admin/post_edit_url', esc_url( add_query_arg( $url_parameters, get_permalink( $page_id ) ) ), $post_id );
        ...
        function get_page_id( $admin_url, $title, $required_capability = null ) {
            ...
            $admin_url = remove_query_arg( array('vgfa_source', 'wpfa_id', 'wpfam'), $admin_url );
            ...
            $post_type_key = $this->get_settings( 'post_type_for_pages', 'page' );
            if ( $url_path === 'index.php' ) {
                $page_id = (int) $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_type = %s AND post_status = 'publish' AND post_content LIKE '%[vg_display_admin_page%' AND post_content LIKE %s", $post_type_key, '%' . $wpdb->esc_like( $url_path ) . '%' ) );
            } else {
                $page_id = (int) $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_type = %s AND post_status = 'publish' AND post_content LIKE '%[vg_display_admin_page%' AND post_content LIKE %s", $post_type_key, '%' . $wpdb->esc_like( $url_path ) . '%' ) );
            }
        ...
        function identify_source_id() {
            ...
            $pagebuilder_parameters = apply_filters( 'wp_frontend_admin/pagebuilder_parameters', array('elementor-preview', 'brizy-edit-iframe') );
            if ( wp_doing_ajax() || !is_admin() && !preg_match( '/(' . implode( '|', $pagebuilder_parameters ) . ')/', $_SERVER['REQUEST_URI'] ) ) {
                return;
            }
        ...
        function get_current_page_settings( $page_id, $key, $default = '' ) {
            ...
            $original_blog_id = WPFA_Global_Dashboard_Obj()->switch_to_dashboard_site();
            $page_settings = $this->get_settings( 'page_settings', array() );
            if ( !empty( $page_settings[$key] ) ) {
                $value = array();
                if ( $key === 'vgfa_text_changes' ) {
                    foreach ( $page_settings[$key] as $meta_row ) {
                        if ( strpos( $meta_row['meta_value'], $page_id ) !== false ) {
                            $value[] = $meta_row['meta_value'];
                        }
                    }
                }
                if ( $key === 'vgfa_disabled_columns' ) {
                    foreach ( $page_settings[$key] as $meta_row ) {
                        if ( strpos( $meta_row['meta_value'], '"' . $page_id . '"' ) !== false ) {
                            $value[] = $meta_row['meta_value'];
                        }
                ...
                if ( $key === 'vgfa_hidden_elements' ) {
                    foreach ( $page_settings[$key] as $meta_row ) {
                        if ( (string) $page_id === $meta_row['post_id'] ) {
                            $value = $meta_row['meta_value'];
                            break;
                        }
                    }
                }
            } elseif ( $key === 'vgfa_text_changes' ) {
        ...
        function render_admin_css( $source_id = null ) {
            ...
            $show_wp_screen_options = (bool) $this->get_settings( 'show_wp_screen_options' );
            if ( get_current_user_id() === (int) $this->get_settings( 'wp_screen_options_user_id' ) ) {
                $show_wp_screen_options = true;
            }
            ?>
            ...
            if ( !empty( $this->get_settings( 'disable_all_admin_notices' ) ) ) {
                ?>
					...
					.vgca-only-admin-content body.wp-admin .updated:not(#message),
					/* Don't hide error notices */
					/* .vgca-only-admin-content body.wp-admin .notice.error, */
					.vgca-only-admin-content body.wp-admin .is-dismissible:not(#message),
					.vgca-only-admin-content #wpbody-content>#hidden-admin-notices-panel>div.error.pa-notice-wrap,
					.vgca-only-admin-content #codepeople-review-banner,
					.vgca-only-admin-content body.wp-admin .notice:not(.error):not(#message){
            ...
            ?>px;
				...
				.vgca-only-admin-content .update-nag,
				.vgca-only-admin-content #contextual-help-link-wrap,
			<?php 
            if ( empty( $show_wp_screen_options ) ) {
                ?>
            ...
            ?>
				...
				/*Fluentcrm*/
				/*The menu breaks if we use display:none*/
				.fluentcrm_main_menu_items .fluentcrm_menu_logo_holder {
					visibility: hidden !important;
				}
				/*Simple admin theme*/
				.vgca-only-admin-content .woocommerce-page .wrap {
            ...
            if ( !empty( $hidden_elements ) ) {
                $cleaned_selectors = '.vgca-only-admin-content ' . implode( ',' . PHP_EOL . ' .vgca-only-admin-content ', array_filter( explode( ',', $hidden_elements ) ) );
                echo wp_strip_all_tags( $cleaned_selectors );
                ?>
			{
					display: none !important;
				}
        ...
        function cleanup_admin_page_for_frontend() {
            // If it's a frontend page, allow it only for ?elementor-preview pages
            $pagebuilder_parameters = apply_filters( 'wp_frontend_admin/pagebuilder_parameters', array('elementor-preview', 'brizy-edit-iframe', 'is-editor-iframe') );
            if ( wp_doing_ajax() || !is_admin() && !preg_match( '/(' . implode( '|', $pagebuilder_parameters ) . ')/', $_SERVER['REQUEST_URI'] ) ) {
                return;
            }
            ...
            if ( !empty( $_GET['vgfa_source'] ) ) {
                ...
                if ( $required_capability ) {
                    ...
                    $required_capability_html .= '</ul>';
                    $required_capability_html .= sprintf( __( '<div><b>Give permission to other user roles</b>. You can use the <a href="%1$s" target="_blank">User Role Editor</a> plugin to assign the capability to the user role required by the page: "%2$s". Careful, assign advanced capabilities only if you trust the users.</div>', VG_Admin_To_Frontend::$textname ), esc_url( VG_Admin_To_Frontend_Obj()->get_plugin_install_url( 'user-role-editor' ) ), esc_html( $required_capability ) );
                }
            }
            ?>
			<script>
            ...
            ?>;
				...
				if (window.parent != window || window.location.href.indexOf('vgfa_internal=1&vgfa_source=') > -1) {
			<?php 
            if ( is_admin() ) {
                ?>
            ...
            $popup_selectors = '.thickbox-loading .media-modal.wp-core-ui,.media-modal, .showwuSweetAlert, .mfp-content, .mce-window.mce-in, #WUB_window, .gh-modal-dialog';
            $popup_selectors .= ( !empty( $this->get_settings( 'extra_popup_selectors' ) ) ? ',' . $this->get_settings( 'extra_popup_selectors' ) : '' );
            $backend_urls_forced_outside_iframe = $this->get_backend_urls_forced_frontend();
            $data = apply_filters( 'vg_admin_to_frontend/backend/js_data', array(
                ...
                'backend_urls_forced_outside_iframe'      => $backend_urls_forced_outside_iframe,
                'stateful_urls_that_require_referer'      => '',
                'disable_all_admin_notices'               => !empty( $this->get_settings( 'disable_all_admin_notices' ) ),
                'stateful_urls_that_dont_require_referer' => $this->get_settings( 'stateful_urls_that_dont_require_referer', '' ),
                ...
                'extra_backend_data_to_report_to_parent'  => array(),
            ) );
            ?>
        ...
        function get_default_post_to_edit( $post_type = 'post', $create_in_db = false ) {
            ...
            if ( $create_in_db ) {
                $post_id = wp_insert_post( array(
                    'post_title'  => ( $post_title ? $post_title : __( 'Auto Draft' ) ),
                    'post_type'   => $post_type,
                    'post_status' => 'draft',
                ), false, false );
