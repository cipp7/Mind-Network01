...
<style>
	...
	.vgca-iframe-wrapper iframe:not(.vgfa-full-screen) {
		max-width: <?php echo esc_attr($max_content_width); ?> !important;
		min-width: auto;
		margin: 0 auto;
	}
...
<div  class="vgfa-is-loading vgca-iframe-wrapper <?php if ($use_desktop_in_mobile) echo 'desktop-as-mobile'; ?>">
	...
	<?php } ?>
	<iframe id="<?php echo esc_attr($iframe_id); ?>" data-source-id="<?php echo (int) get_the_ID(); ?>" data-minimum-height="<?php echo preg_replace('/[^0-9]/', '', $minimum_height_px); ?>" data-lazy-load="<?php echo (bool) $lazy_load; ?>" data-forward-parameters="<?php echo (bool) $forward_parameters; ?>"  data-wpfa="<?php echo esc_url($final_url); ?>" src="<?php echo $lazy_load ? '' : esc_url($final_url); ?>" class="intrinsic-ignore skip-lazy no-lazyload <?php echo sanitize_html_class($class_name); ?>" data-no-lazy="1"></iframe>
...
<script>
	...
	iframeWrappers.forEach(function (iframeWrapper) {
		...
		if (parentTag.match(/(STRONG|I|B|PRE)/)) {
